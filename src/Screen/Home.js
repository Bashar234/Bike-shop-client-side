  ...
  const reportedItem = async (id) => {
    if (window.confirm("Do you want reported")) {
      const { data } = await axios.patch(
        `https://server-six-nu.vercel.app/user/report/${id}`
      );
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      const { data } = await axios.get(
        `https://server-six-nu.vercel.app/all-product`
      );
      setAllProduct(data?.product.reverse());
      setLoder(false);
      const res = await axios.get(`https://server-six-nu.vercel.app/category`);
      ...
      <div>
        ...
        <div className="lg:flex my-10">
          <div className="category mx-4">
            <div className="card mb-5 bg-sky-800 text-white w-60 shadow-xl mx-auto max-w-screen-sm">
